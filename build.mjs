...
const RESULT_SIGNALS =
  /\breduc|\bshorten|\bcut(ting|s)?\b|\benabl|\bunblock|\bapprov|\bdeliver|\bretir|\beliminat|\bsav(ing|ed|es)\b|\bprevent|\bshrank\b|\bfree(d|ing)\b|\bresulting in\b|\bso that\b/i;
  ...
  return `
    <div class="section">
      ...
      <ul class="contact-list">
        ${Object.values(c).filter(Boolean).map((value) => `<li>${esc(value)}</li>`).join("")}
      </ul>
    </div>
    <div class="section">
...
// application presents the candidate as), not a factual one — unlike role titles
...
async function buildAppendix(browser, profile, headline, variantDir, outDir) {
  ...
  const c = profile.contact;
  const contactLine = Object.values(c).filter(Boolean).join("   ·   ");
...
async function main() {
  const { workspaceDir, args } = resolveWorkspace({ root: __dirname });
  const variantSlug = args[0];
  if (!variantSlug) fail("usage: node build.mjs <variant-slug> [--workspace <path>]");

  const variantDir = path.join(workspaceDir, "variants", variantSlug);
  if (!fs.existsSync(variantDir)) fail(`no such variant: ${path.relative(__dirname, variantDir)}`);
  ...
  console.log(`\nDone. Output in ${path.relative(__dirname, outDir)}/`);
}
