#!/usr/bin/env node
// Smoke-test the active workspace's watchlists. No JD fetch, scoring, or writes.
...
const resolved = resolveWorkspace({ root: ROOT });
const PREFS = path.join(resolved.workspaceDir, "inbox/prefs.yaml");
const UA = "cv-generator-watchlist-probe/2.0";
const PAUSE_MS = 80;
const VERBOSE = resolved.args.includes("--verbose") || process.env.VERBOSE === "1";
...
console.log(
  `watchlist probe — ${liveGh.length} greenhouse + ${ashbyTokens.length} ashby from the active workspace` +
...
for (const r of results) {
  ...
  for (const job of r.jobs) {
    const cls = classifyTitle(job.title, prefs);
    if (!cls) continue;
...
const rejected = hits.filter((h) => h.band === "reject");

console.log("\ntitle scan (core/stretch; configured rejects are listed separately)");
...
if (rejected.length) {
  const byWhy = new Map();
  for (const h of rejected) byWhy.set(h.why, (byWhy.get(h.why) || 0) + 1);
  const summary = [...byWhy.entries()].map(([w, n]) => `${w} ×${n}`).join("; ");
  console.log(`\nrejected: ${rejected.length}  (${summary})`);
  if (VERBOSE) {
...
console.log(
  `\nboards: ${results.length - failed}/${results.length} HTTP 200   hits: ${core.length} core, ${stretch.length} stretch, ${rejected.length} rejected`,
);
