---
description: Re-check the evidence bank against the current state of the Work vault
argument-hint: (optional) a specific evidence file to check, e.g. semrush-data-ownership
---

Each file in `data/evidence/` ends with a `Source:` line naming the vault note(s) it was extracted from (paths relative to `~/Documents/Work/PROJECTS/`). Your job is to check whether those dossiers are still accurate — **never auto-write changes**.

Target: $ARGUMENTS (if empty, check every file in `data/evidence/`)

## Steps

1. For each dossier in scope, read its `Source:` line and open the referenced vault note(s) under `~/Documents/Work/PROJECTS/`.
